...
public class DockerRule extends ExternalResource {
    ...
    DockerRule(DockerRuleParams params) {
        ...
        ContainerConfig containerConfig = createContainerConfig(params.imageName, params.ports, params.mappedPorts, params.cmd);
    ...
    private ContainerConfig createContainerConfig(String imageName, String[] ports, String[] mappedPorts, String cmd) {
        Map<String, List<PortBinding>> portBindings = new HashMap<>();
        for (int i = 0; i < ports.length ; i ++) {
            String port = ports[i];
            List<PortBinding> hostPorts = getMappingsFrom(port, i, mappedPorts);
    ...
    private List<PortBinding> getMappingsFrom(String containerPort, int pos, String[] mappedPorts) {
        if (mappedPorts != null && mappedPorts.length > pos) {
            return Collections.singletonList(PortBinding.of("0.0.0.0", mappedPorts[pos]));
        }
        return Collections.singletonList(PortBinding.of("0.0.0.0", containerPort));
    }
